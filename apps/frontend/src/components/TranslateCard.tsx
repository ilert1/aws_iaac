...
export const TranslateCard = ({
	...
	selected: boolean;
	onSelected: (item: ITranslateDbResult) => void;
	translateItem: ITranslateDbResult;
}) => {
	const { deleteTranslation, isDeletingTranslation } = useTranslate();
	...
	return (
		...
		>
			...
			<Button
				...
				onClick={async () => {
					deleteTranslation(translateItem.requestId);
				}}
			>
				{isDeletingTranslation ? <Loading /> : <Trash2 />}
			</Button>
		</Card>
	);
};
